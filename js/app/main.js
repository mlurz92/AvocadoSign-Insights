    ...
    checkDependencies() {
        return new Promise((resolve) => {
            const internalModules = { 
                ...
                publicationService: window.publicationService, uiManager: window.uiManager, 
            ...
            const generatorModules = {
                publicationHelpers: window.publicationHelpers, titlePageGenerator: window.generators?.titlePageGenerator, 
                abstractGenerator: window.generators?.abstractGenerator, introductionGenerator: window.generators?.introductionGenerator, 
                methodsGenerator: window.generators?.methodsGenerator, resultsGenerator: window.generators?.resultsGenerator, 
                discussionGenerator: window.generators?.discussionGenerator, referencesGenerator: window.generators?.referencesGenerator, 
                stardGenerator: window.generators?.stardGenerator
            };
            ...
            for (const dep in generatorModules) {
                if (typeof generatorModules[dep] === 'undefined' || generatorModules[dep] === null) {
                    throw new Error(`Generator module or dependency '${dep}' is not available.`);
                }
            }
