window.generators.resultsGenerator = (() => {
    ...
    function generateComparisonHTML(stats, commonData) {
        ...
        if (interComp && surgeryAloneStats && neoadjuvantStats) {
            ...
            const aucNeoadjuvant = neoadjuvantStats.performanceAS.auc.value;
            interCohortComparisonText = ` Furthermore, there was no significant difference in the diagnostic performance of the Avocado Sign between the treatment-naïve (AUC, ${helpers.formatValueForPublication(aucSurgery, 2, false, true)}) and post-neoadjuvant therapy (AUC, ${helpers.formatValueForPublication(aucNeoadjuvant, 2, false, true)}) cohorts (${helpers.formatPValueForPublication(interComp.pValue)}).`;
        }

        const text = `
            <h3 id="ergebnisse_vergleich_as_vs_t2">Diagnostic Performance and Comparison</h3>
            <p>For the overall cohort (n=${commonData.nOverall}), the Avocado Sign achieved an AUC of ${helpers.formatMetricForPublication(overallStats?.performanceAS?.auc, 'auc')}.${globalCountsText} The interobserver agreement for the sign remained almost perfect (Cohen’s κ = ${helpers.formatValueForPublication(interobserverKappa?.value, 2, false, true)}; 95% CI: ${helpers.formatValueForPublication(interobserverKappaCI?.lower, 2, false, true)}, ${helpers.formatValueForPublication(interobserverKappaCI?.upper, 2, false, true)}) ${helpers.getReference('Lurz_Schaefer_2025')}.${interCohortComparisonText}</p>
            <p>A detailed comparison of the diagnostic performance of the Avocado Sign against both literature-based and data-driven T2 criteria is presented in Table 4. Across clinically relevant subgroups, the Avocado Sign consistently yielded higher discrimination than guideline-derived T2 sets and maintained superiority over the brute-force optimised benchmarks in the neoadjuvant-therapy and overall cohorts.</p>
        `;
