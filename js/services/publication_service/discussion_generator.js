window.discussionGenerator = (() => {
    ...
    function generateDiscussionHTML(stats, commonData) {
        ...
        if (esgarHybridComparison?.delong?.pValue > window.APP_CONFIG.STATISTICAL_CONSTANTS.SIGNIFICANCE_LEVEL) {
            const power = esgarHybridComparison.delong.power;
            if (isFinite(power) && power < 0.8) {
                powerAnalysisText = ` A post-hoc power analysis revealed that this comparison had a power of only ${helpers.formatValueForPublication(power * 100, 0)}% to detect the observed difference in AUCs, suggesting that the study may have been underpowered for this specific comparison.`;
            }
        }
