window.insightsTab = (() => {
    ...
    function _renderMismatchAnalysis(allStats, processedData) {
        ...
        const createCell = (label, count, bgColor, action, tooltip) => `
            <div class="mismatch-cell ${bgColor} p-3 rounded d-flex flex-column justify-content-center align-items-center" data-action="${action}" data-mismatch-key="${action.replace('show-','')}" data-tippy-content="${tooltip}" style="cursor: pointer;">
        ...
        const html = `
            <div class="mismatch-grid">
                ...
                <div class="mismatch-header-item text-center small">Correct</div>
                ${createCell(texts.concordantCorrect, mismatch.concordantCorrect.length, 'bg-success-subtle', 'show-mismatch-details', texts.tooltip_concordantCorrect)}
                ${createCell(texts.asSuperior, mismatch.asSuperior.length, 'bg-primary-subtle', 'show-mismatch-details', texts.tooltip_asSuperior)}
                <div class="mismatch-header-item text-center small">Incorrect</div>
                ${createCell(texts.t2Superior, mismatch.t2Superior.length, 'bg-warning-subtle', 'show-mismatch-details', texts.tooltip_t2Superior)}
                ${createCell(texts.concordantIncorrect, mismatch.concordantIncorrect.length, 'bg-danger-subtle', 'show-mismatch-details', texts.tooltip_concordantIncorrect)}
            </div>`;
    ...
    function _renderView(allStats, processedData){
        ...
        switch(insightsView) {
            ...
            case 'feature-importance':
                ...
                cardHTML = window.uiComponents.createStatisticsCard('feature-importance', cardTitle, window.uiComponents.createFeatureImportanceCardHTML(), false);
                contentArea.innerHTML = `<div class="row justify-content-center"><div class="col-xl-10">${cardHTML}</div></div>`;
